...
sample_watershed <- read.csv(here("Data", "sample_watershed_WBET.csv"))
...
WBET_mean <- mean(sample_watershed$TC_ET_mm, na.rm = TRUE)
  
# Compute for mean - 2SD (standard deviation)
WBET_sd <- 2 * sd(sample_watershed$TC_ET_mm, na.rm = TRUE)
...
# Abnormal data, low:
WBET_abnormal_low <- sample_watershed %>% 
  filter(!is.na(TC_ET_mm)) %>% 
  filter(TC_ET_mm < Threshold_low)
  
# Abnormal data, high:
WBET_abnormal_high <- sample_watershed %>% 
  filter(!is.na(TC_ET_mm)) %>% 
  filter(TC_ET_mm > Threshold_high)
